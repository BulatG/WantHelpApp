...
typealias EditPostHandler = (EventEntity) -> Void
...
class DetailEventViewController: UIViewController {
    ...
    var post:EventEntity!
    var onEditPostHandler: EditPostHandler!
    
    override func viewDidLoad() {
        super.viewDidLoad()
        self.tabBarController?.tabBar.isHidden = true
        mainImage.image = UIImage(named: "mainImage1")
        ...
        mainImage.image = UIImage(named: post!.mainImage ?? "image")
        image2.image = UIImage(named: post!.image2 ?? "image")
        image3.image = UIImage(named: post!.image3 ?? "image")
        descriptionFromCharityVC.text = post?.des2
