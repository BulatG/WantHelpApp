...
final class CoreDataManager {
    ...
    func transformCategoryToObject(completion: @escaping ((Result<[CategoryEntity], Error>) -> Void)) {
        coreDataQueue.async { [self] in
            ...
            do {
                let objects = try viewContext.fetch(fetchRequest)
                let newDataArray: [CategoryEntity]
    ...
    func setCategoriesToStorage(_ categories: [Category], completion: @escaping (Result<[CategoryEntity], Error>) -> Void) {
        DispatchQueue.global(qos: .background).async { [weak self] in
            guard let self = self else { return }
            do {
                try self.setCategoriesToStorage(categories)
                let categories = try self.getCategoriesFromStorage()
    ...
    func setEventsToStorage(_ events: [Event], completion: @escaping (Result<[EventEntity], Error>) -> Void) {
        DispatchQueue.global(qos: .background).async { [weak self] in
            guard let self = self else { return }
            do {
                try self.setEventsToStorage(events)
                let events = try self.getEventsFromStorage()
    ...
    private func setCategoriesToStorage(_ categories: [Category]) throws {
        let fetchRequest: NSFetchRequest<Categories> = Categories.fetchRequest()
        let categoriesCount = try viewContext.fetch(fetchRequest)
        if categoriesCount.isEmpty {
            let category = Categories(context: self.viewContext)
            ...
            category.id = Int16((categories[0].id))
            category.image = categories[0].image
            
            category2.name = categories[1].name
            category2.image = categories[1].image
            category2.id = Int16((categories[1].id))
            
            category3.name = categories[2].name
            category3.image = categories[2].image
            category3.id = Int16((categories[2].id))
            
            category4.name = categories[3].name
            category4.image = categories[3].image
            category4.id = Int16((categories[3].id))
            
            category5.name = categories[4].name
            category5.image = categories[4].image
            category5.id = Int16((categories[4].id))
        }
        try self.viewContext.save()
    }
    
    private func getCategoriesFromStorage() throws -> [CategoryEntity] {
        let fetchRequest: NSFetchRequest<Categories> = Categories.fetchRequest()
        let objects = try viewContext.fetch(fetchRequest)
        let newDataArray: [CategoryEntity]
        dataArray.append(contentsOf: objects)
    ...
    private func setEventsToStorage(_ events: [Event]) throws {
        let fetchRequest: NSFetchRequest<Events> = Events.fetchRequest()
        let eventsCount = try viewContext.fetch(fetchRequest)
        if eventsCount.isEmpty {
            let event = Events(context: viewContext)
            ...
            event.adress = events[0].adress
            event.des = events[0].description
            event.desTwo = events[0].description2
            event.footer = events[0].footer
            event.imaageTwo = events[0].image2
            event.fundName = events[0].fundName
            event.imageThree = events[0].footer
            event.mainImage = events[0].mainImage
            event.id = Int16(events[0].id)
            event.title = events[0].title
            event.phone = events[0].phone
            
            event2.adress = events[1].adress
            event2.des = events[1].description
            event2.desTwo = events[1].description2
            event2.footer = events[1].footer
            event2.imaageTwo = events[1].image2
            event2.fundName = events[1].fundName
            event2.imageThree = events[1].footer
            event2.mainImage = events[1].mainImage
            event2.id = Int16(events[1].id)
            event2.title = events[1].title
            event2.phone = events[1].phone
        }
        try self.viewContext.save()
    }
    private func getEventsFromStorage() throws -> [EventEntity] {
        let fetchRequest: NSFetchRequest<Events> = Events.fetchRequest()
        let objects = try viewContext.fetch(fetchRequest)
        let newDataArray: [EventEntity]
        eventsEntityArray.append(contentsOf: objects)
        newDataArray = eventsEntityArray.map { EventEntity(
            des: $0.des,
            title: $0.title,
            id: $0.id,
            footer: $0.footer,
